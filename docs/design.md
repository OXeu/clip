...
- 导入、导出和播放按钮使用从 Clip logo 取出的浅粉色；导出按钮的文字、图标与下拉箭头为白色，其他主按钮搭配深色文字。播放头和键盘焦点保留更深的玫瑰色以便辨认。选中轨道与下拉选项使用中性灰，选中片段增加中性描边以区分相邻素材。
...
- 采用深色文字的主按钮在明暗主题下对比度均约 8.8:1，悬停和按下时不低于 7.4:1。导出按钮在悬停和按下时保持白色前景，禁用时使用灰色；高对比度模式改用系统颜色。
