...
顶部“更多”菜单中的“导出项目”会下载一份 `.clip` 文件，其中包含轨道、片段、速度、命名、绑定关系和工作位置，但不包含原视频。通过“导入项目”、`Ctrl+Shift+O` 或直接拖入 `.clip` 文件，可以在刷新、换设备后继续剪辑，也可以与 Windows 桌面版互相传递项目。

恢复时浏览器仍会要求重新选择原视频，并使用与刷新恢复相同的轻量 SHA-256 采样核对内容。选择错误文件不会替换当前项目；取消文件恢复时，原先正在编辑的项目也会保持不变。
