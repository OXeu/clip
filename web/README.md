...
顶部“更多”菜单中的“导出项目”会下载一份 `.clip` 文件，其中包含轨道、片段、速度、命名、绑定关系和工作位置，但不包含原始素材。通过“导入项目”、`Ctrl+Shift+O` 或直接拖入 `.clip` 文件，可以在刷新、换设备后继续剪辑，也可以与 Windows 桌面版互相传递项目。
...
双击编辑文字。右键字幕轨可设置顶对齐、矩形中心对齐或底部对齐。

选中字幕后，预览画面会显示可拖动、四角可缩放的字幕矩形框；水平或垂直居中时会显示
吸附线。字幕换行宽度不会超过矩形框，文本块高度按内容自然增长。WebCodecs 与
