...
2. 在时间轴点击或拖动定位播放头，按 `S` 分割。单击片段选择，按 `Delete`，或右键选择 **Delete · 删除片段**。剩余片段自动收拢，源文件不会改变。
3. 按空格预览剩余内容；左右方向键按标称帧率逐帧移动。用 `Ctrl+Z` 撤销，`Ctrl+Y` / `Ctrl+Shift+Z` 重做。可以删除所有片段，再通过撤销恢复。
