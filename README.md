...
使用 **C# / .NET 10 + WPF** 构建的多素材、多轨道剪辑工具：主轨负责导出，候选轨用于挑选和剪切素材。视频探测、缩略图、兼容预览转换和导出由 FFmpeg 驱动；窗口与播放控件使用 Windows 原生桌面技术，界面遵循 Fluent 2，操作图标采用 Remix Icon。
...
2. 选中任意轨道上的片段即可预览该素材。空格播放 / 暂停；片段结束后沿同一轨道继续播放下一段，必要时自动切换素材，不会跳到其他轨道。主编辑区中，按钮和缩放滑块不会将空格误当点击；文字输入框仍保留文字编辑行为。
3. 按 `S` 分割当前轨道播放头处的片段，播放中分割不会中断播放。按 `Delete` / `X`，或右键“删除片段”删除选中片段。
4. 拖动片段可跨轨移动，或在同轨内重排。插入线标明落点；当前采用顺序插入、轨内自动收拢，不允许重叠或留黑场。移入主轨的片段参与导出，移出主轨后不再导出。
5. 左侧“片段速度”可选择预设或输入任意 **0.1–8 倍**的小数，按“应用”或 Enter 生效。速度只作用于当前片段，并同步改变时间轴长度；源视频入点 / 出点仍按原素材时间显示。
6. `Ctrl+Z` 撤销，`Ctrl+Y` / `Ctrl+Shift+Z` 重做；导入、跨轨移动、排序、分割、删除和倍速均支持项目级撤销。左右方向键按当前片段的源视频帧率与速度换算后逐帧定位。
7. 主轨非空时点击“导出视频”。只合并主轨，候选轨的视频和声音均不导出。选择新的 MP4 文件名；可取消，失败和取消会清理临时输出。
...
轨道较多时，可使用右侧滚动条、`Alt + 滚轮`，或在轨道名称区滚动来上下浏览；拖拽靠近视口边缘会自动滚动。轨道名称和共享标尺始终保持可见。

### 导出选项

| 选项 | 行为 |
| --- | --- |
| 原画 | 以主轨首个素材的分辨率为基准，高质量 H.264 编码，CQ/CRF 16；奇数宽高补齐为偶数 |
...
“原画”是高质量重新编码，**不是无损码流复制**。输出分辨率和帧率默认参考主轨首个素材；不同尺寸按比例缩放、补边，不同帧率归一后再拼合。任意位置精确切割不受关键帧限制；速度通过视频时间戳和音频 `atempo` 同步调整，导出音频保持音调。只读取每个素材的首个普通视频流和首个音频流，不导出字幕或封面。主轨混合有声和无声素材时，无声部分补静音；主轨全部无声时不生成音轨。音频输出为 48 kHz、双声道 AAC 192 kbps。极短片段的最终时长受输出帧率限制，至少保留一帧。目标尺寸是像素宽高，不是指定文件大小。
